...
// user object, dispatch function coming from useStateValue
  ...
  const login = async () => {
  if (!user) {
// in successful login we are getting some response n that response return entire thing
    const {
      user : { refreshToken, providerData },
    } = await signInWithPopup(firebaseAuth, provider);
    dispatch({
      type : actionType.SET_USER,
// zero index and supplying to userState
      user : providerData[0],
    });
  ...
  return (
    ...
    <div className="hidden md:flex w-full h-full items-center justify-between">
      <Link to={"/"} className="flex items-center gap-2">
        ...
        <p className="text-headingColor text-xl font-bold">Darjeeling</p>
      </Link>
      ...
      whileTap={{ scale: 0.6 }}
       src={user ? user.photoURL : Avatar }
        ...
        />
        {
         ...
         {
           user && user.email === "dichensherpa59@gmail.com" && (
           <Link to={"/createItem"}>
        ...
        />
        {
             ...
             className="w-40 bg-gray-50 shadow-xl rounded-lg flex flex-col absolute top-12 right-0 ">
         {
          ...
          <li className="text-base text-textColor hover:text-headingColor duration-100 transition-all ease-in-out cursor-pointer hover:bg-slate-100 px-4 py-2 "onClick={() => setIsMenu(false)}>Service</li>
           ...
           <p className="m-2 p-2 rounded-md shadow-md flex items-center justify-center bg-gray-200 gap-3 cursor-pointer hover:bg-gray-300 transition-all duration-100 ease-in-out text-textColor text-base" onClick={logout} >Log Out<MdLogout /></p>
